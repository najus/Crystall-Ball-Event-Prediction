...
runJob(){
	START=$(date +%s)
	case $1 in
		1)  $HADOOP jar $JAR edu.mum.crystalball.wordcount.WordCountHBaseDriver /wordcount/input/wordcount.txt
		 	;;
		2)  $HADOOP jar $JAR edu.mum.crystalball.pairs.PairsHBaseDriver /relativefrequency/input/sample.txt
		 	;;
		3)  $HADOOP jar $JAR edu.mum.crystalball.stripes.StripesHBaseDriver /relativefrequency/input/sample.txt
		 	;;
		4)  $HADOOP jar $JAR edu.mum.crystalball.hybrid.HybridHBaseDriver /relativefrequency/input/sample.txt
		    ;;
	esac
